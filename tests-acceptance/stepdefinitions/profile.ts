...
defineSupportCode(function ({ Given, When, Then }) {
    Given(/^I am at the profile page$/, async () => {
        await browser.get("http://localhost:4200/profile");
        await expect(browser.getTitle()).to.eventually.equal('MusicMix');
    })

    When(/^I enter the option to edit profile$/, async () => {
        await element(by.buttonText('edit profile')).click();
    });

    When(/^I try to change my name to "([^\"]*)"$/, async (name) => {
        await $("input[name='name']").sendKeys(<string> name);
    });

    When(/^I accept my changes$/, async() => {
        await element(by.buttonText('accept changes')).click();
    });

    Then(/^my profile name is changed to "([^\"]*)"$/, async (name) => {
        await request.get(base_url + "musicians")
        .then(body => 
        expect(body.includes(`"name":"${name}"`)).to.equal(true));
    });
    ...
    //     var options:any = {method: 'POST', uri: (base_url + "musicians"), body:aluno, json: true};
